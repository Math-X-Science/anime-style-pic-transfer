...
if __name__ == "__main__":
    ...
    if "video" in runtime_json:
        ...
        video_json = {"input": "video.mp4", "output": "output.mp4", "audio": True}
    fps = input("\n输入视频帧率(默认15)：")
    if is_empty(fps):
        fps = 15
    else:
    ...
    if new_cfg:
        ...
        if is_empty(resize_mode) or (resize_mode != "1" and resize_mode != "2"):
            ...
            width = input("\n输入图片宽度：")
            height = input("\n输入图片高度：")
            setting_resize = {"width": int(width), "height": int(height)}
        ...
        if is_empty(tag_mode) or tag_mode == "1":
            tag_json = {"enable": True, "mode": ""}
        elif tag_mode == "2":
            tag_json = {"enable": True, "mode": "action"}
            actions = input(
            ...
            tag_json = {"enable": True, "mode": "action_common"}
            actions = input(
                "\n输入动作表情tag列表，多个tag用逗号分隔(默认:open,opened,close,closed,blink,smile,laugh,behind)：\n"
            )
            if is_empty(actions):
    ...
    runtime_json["config"] = {"setting": "img_base.txt", "webui": ct}
    ...
    print(
        "\n配置文件已生成，请复制文件路径，在 启动_自动模式 或 启动_交互模式 下输入路径来启动工作流。路径："
        + config_file
    )
