...
from converter.workflow.image_utils import round_up
...
class WebuiConfig:
            ...
            for item in controlnet_args:
                ...
                for model in models:
                    if model_name in model:
                        ...
                        matched = True
                        print(
                            "警告: 找不到 controlnet 模型 %s, 将使用 %s 代替"
                            % (model_name, model)
                        )
                        break
                if not matched:
    ...
    def inflate_control_nets(
        self, cur_img_ori, last_img_tra=None, cur_mask_ori=None, last_mask_tra=None
    ):
