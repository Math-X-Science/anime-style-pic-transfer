...
def convert_images_to_video(image_path, output_file, fps, bit_rate):
    ...
    subprocess.run(
        [
            ...
            str(fps),
            "-i",
            os.path.join(image_path, "%05d.png"),
            "-c:v",
            "libx264",
            "-pix_fmt",
            "yuv420p",
...
if __name__ == "__main__":
    ...
    if video_config.enable_audio():
        ...
        silent_file = os.path.join(
            output_path.parent, output_path.stem + "_silent" + output_path.suffix
        )
        music_file = os.path.join(
            output_path.parent, output_path.stem + ".m4a"
        )  # 提取视频音乐文件
    ...
    convert_images_to_video(
        workspace.output + "/imgs/", silent_file, fps, bit_rate=str(bit_rate)
    )
