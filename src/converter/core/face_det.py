...
ort_session = onnxruntime.InferenceSession(
    model_args.Retina_face_model_path,
...
def detect_face(
    img, resize=1, confidence_threshold=0.8, top_k=50, nms_threshold=0.4, keep_top_k=20
):
    # resize
    height, width = img.shape[0], img.shape[1]
    max_edge = max(width, height)
    scale_factor = 840 / max_edge if max_edge > 840 else 1.0
    ...
    # if scale
    img -= np.array([104.0, 117.0, 123.0])  # BGR
