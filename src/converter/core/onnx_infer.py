...
def argsparse():
    ...
    parser.add_argument(
        ...
        type=str,
        default="/data/",
        help="Input image or image folder or mp4 video file. Default: inputs/whole_imgs",
    )
    parser.add_argument(
        "-o", "--output", type=str, default="./USA", help="Output folder. Default: USA"
    )
    parser.add_argument(
        "-m", "--model_path", type=str, help="onnx path for style model"
    )
    parser.add_argument(
        "-b",
        "--background",
        action="store_true",
        help="Whether to convert the background",
    )
    parser.add_argument(
        "-c", "--save_croped", action="store_true", help="save the croped face"
    )
    parser.add_argument(
        "-t",
        "--IfConcat",
        ...
        help="Whether to splice the original video with the converted video",
    )
    parser.add_argument(
        "-p",
        "--use_parse",
        type=bool,
        default=True,
        help="Segmentation face. Default: True",
    )
...
def Conversion(img, ort_session, background, use_parse):
    ...
    cropped_faces, affine_matrices = faceRestoreHelper.align_warp_face(
        img, all_landmarks_5, face_size=512
    )
    # face restoration
    restored_faces = []
    ...
    inverse_affine_matrices = faceRestoreHelper.get_inverse_affine(
        affine_matrices, upscale_factor=1
    )
    restored_img = faceRestoreHelper.paste_faces_to_image(
        img,
        restored_faces,
...
def image_enforce(
    ...
    save_croped=save_croped,
    use_parse=use_parse,
):
    ort_session = ort.InferenceSession(
        model_path, sess_options=ort_sess_options, providers=providers
    )
    img_list = get_image_file_list(input)
    # 如果output的结尾是.jpg,就删除该文件如果存在。
    if output[-3:] == "jpg":  # Check if the file exists before trying to delete
        try:
            os.remove(output)
            print(f"File {output} deleted successfully.")
        except OSError as e:
            print(f"Error deleting file {output}: {e}")
    else:
    ...
    for img_path in tqdm(img_list):
        ...
        cropped_faces, restored_faces, restored_img = Conversion(
            input_img, ort_session, background, use_parse
        )
        # save faces
        # if save_croped:
        ...
        if restored_img is not None:
            if output[-3:] == "jpg":
                save_restore_path = output
                faceRestoreHelper.imwrite(restored_img, save_restore_path)
            else:
                save_restore_path = os.path.join(output, img_name)
                faceRestoreHelper.imwrite(restored_img, save_restore_path)
...
def video_enforce(
    ...
    ouput_sound_path = os.path.join(output, "sound.mp3")
    if IfConcat == "Horizontal":
        video_out = cv2.VideoWriter(
            ouput_video_path, cv2.VideoWriter_fourcc(*"mp4v"), fps, (width * 2, height)
        )
    elif IfConcat == "Vertical":
        video_out = cv2.VideoWriter(
            ouput_video_path, cv2.VideoWriter_fourcc(*"mp4v"), fps, (width, height * 2)
        )
    else:
        video_out = cv2.VideoWriter(
            ouput_video_path, cv2.VideoWriter_fourcc(*"mp4v"), fps, (width, height)
        )
    ...
    while True:
        ...
        if not ret:
            break
        cropped_faces, restored_faces, restored_img = Conversion(
            frame, ort_session, background, use_parse
        )
    ...
    try:
        ...
        ouput_videoSounds_path = ouput_video_path.rsplit(".", 1)[0] + "_sound.mp4"
        command = [
            "ffmpeg",
            "-loglevel",
            "error",
        ...
        r = subprocess.check_call(
            command
        )  # Merge the output video with the sound to get the final result
    except:
