...
def convert_images_to_video(image_path, output_file, fps, bit_rate):
    # 调用FFmpeg将图片合成为视频
    subprocess.run([
        ffmpeg_path, "-y", "-framerate", str(fps), "-i", os.path.join(image_path, '%05d.png'),
...
def attach_music(output_file, video_file, music_file):
    # 调用FFmpeg将音乐合成进视频
    subprocess.call(
        [ffmpeg_path, '-i', video_file, '-i', music_file, '-c:v', 'copy', '-c:a', 'aac', '-map', '0:v:0',
         '-map', '1:a:0', '-y', output_file])


def get_bit_rate(video_file):
    json_str = subprocess.check_output(
        [ffprobe_path, "-v", "error", "-print_format", "json", "-show_entries", "stream=bit_rate", video_file])
