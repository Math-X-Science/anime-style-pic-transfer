...
class BetaAndGamma(nn.Module):
    ...
    def __init__(self, n_features: int, feature_size: tuple, light: bool) -> None:
        super().__init__()
        ...
        if light:
            ...
            # 根据实际的高度和宽度计算全连接层的输入大小
            fully_connection_layers = [
                nn.Flatten(),
                nn.Linear(feature_height * feature_width * n_features, n_features, False),
                nn.ReLU(True),
                nn.Linear(n_features, n_features, bias=False),
                nn.ReLU(True),
            ]
...
class Generator(nn.Module):
    ...
    def __init__(self, input_nc: int, output_nc: int, n_hiddens=64, n_resblocks=6, img_size=(512,512), light=False) -> None:
        super().__init__()
        ...
        self.beta_gamma = BetaAndGamma(
                                        n_hiddens * mult, 
                                        (self.img_height // mult, self.img_width // mult),  # 确保这里是一个元组
                                        light=self.light
                                    )
